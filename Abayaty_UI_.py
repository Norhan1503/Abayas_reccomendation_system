...
# -------------------------------------
# SIMPLIFIED image loading - remove optimizations that cause issues
# -------------------------------------
...
def preload_images_parallel(url_list):
    images = {}
    # Use simpler approach without batching
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        future_to_url = {executor.submit(load_single_image, url): url for url in url_list}
...
# Preload images - SIMPLIFIED without spinner
if len(page_data) > 0:
...
# -------------------------------------
st.sidebar.write("---")
st.sidebar.subheader("📊 Feedback Analytics")

feedback_stats = get_feedback_stats()
st.sidebar.metric("Total Ratings", feedback_stats["total_ratings"])
st.sidebar.metric("Average Rating", f"{feedback_stats['average_rating']:.1f} ⭐")
...
if st.sidebar.button("View All Feedback"):
    if "user_feedback" in st.session_state and st.session_state.user_feedback:
        ...
        for i, feedback in enumerate(st.session_state.user_feedback[-5:]):
            st.sidebar.write(f"⭐ {feedback['rating']}/5 - {feedback.get('comment', 'No comment')}")
    else:
...
st.markdown("<h1 style='text-align:center;font-weight:bold;color:#8b5e3c;'>Abayaty 🖤</h1>", unsafe_allow_html=True)
...
# -------------------------------------
# DETAILS PAGE - SIMPLIFIED image loading
# -------------------------------------
    ...
    if idx is None or idx not in df.index:
        ...
        # SIMPLIFIED product image loading - no spinner
        ...
        # Recommendations with SIMPLIFIED image loading
        recs = find_similar(idx)
        st.subheader("You May Also Like")

        # Preload recommendation images - SIMPLIFIED
